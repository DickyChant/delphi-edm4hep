...
# Single source of truth for the key4hep release: the KEY4HEP_RELEASE pin in
# CMakeLists.txt. Sourcing anything else would fail the configure-time check.
KEY4HEP_RELEASE=$(sed -n 's/^set(KEY4HEP_RELEASE "\([^"]*\)".*/\1/p' \
...
echo "Using key4hep release ${KEY4HEP_RELEASE} (from the CMakeLists.txt pin)"
