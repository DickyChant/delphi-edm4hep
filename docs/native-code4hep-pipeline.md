...
| Central tracking | scheduled TPC row aggregation, deterministic IP-constrained helix seed, VD association, and exclusive ID/OD drift-side selection with persistent hit relations | multi-track pattern recognition, sector-boundary merging, unconstrained global refit, outlier rejection, material effects, track truth |
